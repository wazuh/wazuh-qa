...
class Config():
    ...
    def __read_include_paths(self):
        if 'Include paths' not in self.__config_data:
    ...
    def __read_include_regex(self):
        if 'Include regex' not in self.__config_data:
            raise Exception("Include regex is empty")
        self.include_regex = self.__config_data['Include regex']

    def __read_ignore_folders(self):
        if 'Ignore folders' in self.__config_data:
            self.ignore_folders = self.__config_data['Ignore folders']
