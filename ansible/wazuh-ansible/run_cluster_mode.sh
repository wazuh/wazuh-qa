...
paths=( "molecule/default/" "molecule/worker/" "molecule/elasticsearch/" "molecule/elasticsearch-xpack/" "molecule/kibana/" )
...
if [ -z "$IMAGE" ]
then
      echo "Platform not selected. Please select a platform of [bionic, xenial or centos7]. => Aborting"
      exit
else
        for i in "${paths[@]}"
        do
