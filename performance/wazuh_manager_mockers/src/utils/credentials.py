...
- `create_certificate(public_key, private_key, country, province, locality, org, commom_name)`: Creates a
   self-signed X.509 certificate using the provided public and private keys.
- `write_private_key(private_key, credentials_path, private_key_pem_name)`:
   Serializes the EC private key and writes it to a PEM file.
- `write_public_key(public_key, credentials_path, public_key_pem_name)`:
   Serializes the EC public key and writes it to a PEM file.
- `write_certificate(cert, credentials_path)`: Serializes the X.509 certificate and writes it to a PEM file.
- `create_manager_credentials(credentials_path)`: Generates a private key, public key, and self-signed certificate,
...
from cryptography.x509 import Certificate, CertificateBuilder, Name, NameAttribute, random_serial_number
...
def create_certificate(public_key: ec.EllipticCurvePublicKey,
                       private_key: ec.EllipticCurvePrivateKey,
                       country: str = 'US', province: str = 'California',
                       locality: str = 'San Francisco', org: str = 'Wazuh',
...
def write_private_key(private_key: ec.EllipticCurvePrivateKey,
                      credentials_path: str, private_key_pem_name: str = 'private_key.pem') -> None:
...
def write_public_key(public_key: ec.EllipticCurvePublicKey,
                     credentials_path: str, public_key_pem_name: str = 'public_key.pem') -> None:
    ...
    with open(os.path.join(credentials_path, public_key_pem_name), 'wb') as public_file:
...
def create_manager_credentials(credentials_path: str) -> None:
    """Generate a set of manager credentials.

    Create the private key, public key, and a self-signed certificate, and write them to specified files.
