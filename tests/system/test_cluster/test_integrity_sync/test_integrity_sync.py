...
cluster_json_values = [{'key': ['intervals', 'communication', key], 'value': val} for key, val in configuration.items()]
...
def clean_files():
    """Remove all files that the test will create."""
...
def clean_and_restart():
    """Remove invalid rule files and restart managers."""
    yield
...
def test_missing_file(clean_files):
    ...
    # Check that files which should not be synchronized are not sent to the workers.
...
def test_zip_size_limit(clean_files, update_cluster_json, clear_cluster_logs, clean_and_restart):
