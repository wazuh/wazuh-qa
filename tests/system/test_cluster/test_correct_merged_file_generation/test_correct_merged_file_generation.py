...
test_cases_yaml = read_yaml(os.path.join(local_path, 'data/cases_correct_merged_file_generation.yml'))
...
def trigger_and_check_merged(test_case):
    ...
    trigger_value = test_case['test_case'][0]['trigger_value']
    file_content = test_case['test_case'][0]['content']
    number_of_files = test_case['test_case'][0]['number']
    test_type = test_case['test_case'][0]['test_type'] 
    folder = test_case['test_case'][0]['folder']
    file_name = test_case['test_case'][0]['name']
    content = test_case['test_case'][0]['content']

    if trigger_value == "remove":
        host_manager.run_command(testinfra_hosts[0], f'rm {WAZUH_PATH}/etc/shared/default/merged.mg -f')

    elif trigger_value == "add_files":
        if file_content == 'zero':
            if number_of_files == '1':
                host_manager.run_command(testinfra_hosts[0], 
                                         f"touch {WAZUH_PATH}/etc/shared/{folder}/{file_name}.txt")
            elif number_of_files == 'several':
                for file in file_name:
                    host_manager.run_command(testinfra_hosts[0], 
                                             f"touch {WAZUH_PATH}/etc/shared/{folder}/{file}.txt")      
        elif file_content != 'zero':
            if number_of_files == '1':
                host_manager.run_command(testinfra_hosts[0], 
                                         f"touch {WAZUH_PATH}/etc/shared/{folder}/{file_name}.txt")
    ...
    elif test_type == 'start' and trigger_value == 'remove':
...
def test_correct_merged_file_generation(test_case, cleaning_environment, trigger_and_check_merged, restart_or_sleep):
    '''
        ...
        parameters:
            ...
            - trigger_and_check_merged:
                ...
                Also checks merged if the case is required.         
            - restart_or_sleep:
                type: function
    ...
    file_name = test_case['test_case'][0]['name']
    folder = test_case['test_case'][0]['folder']
    malformed_value = test_case['test_case'][0]['malformed_value']
    log_content = test_case['test_case'][0]['log_content']
    content = test_case['test_case'][0]['content']
    ...
    if file_name is not None:
        if type(file_name) == str:
            ...
            assert counter_files == len(file_name)
