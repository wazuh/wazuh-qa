...
test_cases_file_path = os.path.join(test_data_path, 'test_cases', 'cases_recurrent_attacks_before_timeout_expired.yaml')
ossec_log = os.path.join(gettempdir(), 'test_recurrent_attacks_before_timeout_expired', 'ossec.log')
...
def test_recurrent_attacks_before_timeout_expired(configure_environment, metadata, get_information):
    '''
    description: Check that active-response repeated_offenders blocks the attacks for the respective timeout if the
                 attack proceeds before the repeated_offenders timeout has expired.
