...
with open('check_recursion_level_delete.json') as f:
...
def test_check_recursion_level_delete():
    found = False
    ...
    for alert in alerts_json:
        ...
        if alert == test_alert:
            found = True
        if alert.get("syscheck",{}).get("path",{}) == "/fim_test/testing_directory_1/testing_directory_2/check_recursion_level_2_file.txt":
            found = False
            break
    assert found
