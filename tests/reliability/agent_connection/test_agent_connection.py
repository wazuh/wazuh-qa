...
def test_agent_connection(get_report):
    ...
    if phases:
        for phase in phases:
            ...
            if not wazuh_target_report_remoted[phase]['min_tcp_sessions'] == get_report['metadata']['n_agents']:
            ...
            if not wazuh_target_report_agentd[phase]['ever_disconnected'] == 0:
                error_messages += ['Some agents have disconnected']

            if not wazuh_target_report_agentd[phase]['ever_pending'] == 0:
                error_messages += ['Some agents have change to pending status']

            if not wazuh_target_report_agentd[phase]['begin_status']['connected'] == \
               wazuh_target_report_agentd[phase]['end_status']['connected'] == \
