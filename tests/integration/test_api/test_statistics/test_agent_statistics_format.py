...
from wazuh_testing.tools.file import read_json_file, read_file
...
CONFIGURATIONS_PATH = os.path.join(TEST_DATA_PATH, 'configuration_template', 'agent_statistics_format_test_module')
TEST_CASES_PATH = os.path.join(TEST_DATA_PATH, 'test_cases', 'agent_statistics_format_test_module')
...
def test_agent_statistics_format(metadata, restart_wazuh_daemon_function):
    """
    ...
    """
    connected_log = r".*Connected to the server \(\[(\d+\.\d+\.\d+\.\d+)\]:(\d+)/(\w+)\)\."
    obtained_log = evm.check_event(callback=connected_log, file_to_monitor=LOG_FILE_PATH,
                                   timeout=30, error_message='The alert has not occurred').result()
    manager_ip = obtained_log.group(1)

    keys = read_file(CLIENT_KEYS_PATH)
    agent_id = keys.split()[0]

    endpoint = f"/agents/{agent_id}/daemons/stats?daemons_list={metadata['endpoint']}"
