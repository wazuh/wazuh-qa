...
t2_configurations_path = os.path.join(CONFIGURATIONS_PATH, 'configuration_restrict_ignore_regex_values.yaml')
t2_cases_path = os.path.join(TEST_CASES_PATH, 'cases_restrict_ignore_regex_values.yaml')
...
def test_restrict_ignore_regex_values(configuration, metadata, new_file_path, truncate_monitored_files,
    ...
    '''
    description: Check if logcollector reads or ignores a log according to a regex configured in the restrict and restrict tag tag for a
    ...
    if metadata['matches'] == 'restrict':
        log_found = False
        with pytest.raises(TimeoutError):
            log_found = evm.check_ignore_restrict_messages(message=log, regex=metadata['restrict_regex'], tag='restrict',
    ...
    if 'ignore' in metadata['matches']:
        evm.check_ignore_restrict_messages(message=log, regex=metadata['ignore_regex'], tag='ignore', prefix=LOG_COLLECTOR_DETECTOR_PREFIX)
    ...
    if metadata['matches'] == None:
        evm.check_ignore_restrict_messages(message=log, regex=metadata['restrict_regex'], tag='restrict', prefix=LOG_COLLECTOR_DETECTOR_PREFIX)
        log_found = False
        with pytest.raises(TimeoutError):
            log_found = evm.check_ignore_restrict_messages(message=log, regex=metadata['ignore_regex'], tag='ignore',
                                                           prefix=LOG_COLLECTOR_DETECTOR_PREFIX)
