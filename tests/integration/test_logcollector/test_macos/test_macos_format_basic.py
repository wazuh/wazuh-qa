...
def test_macos_format_basic(get_configuration, configure_environment, get_connection_configuration,
    ...
    if log_command == 'logger':
        ...
        logcollector.generate_macos_custom_log(macos_message['type'], macos_message['level'], macos_message['subsystem'], macos_message['category'])
        expected_macos_message = logcollector.format_macos_message_pattern(
                                                        'custom_log',
                                                        logcollector.TEMPLATE_OSLOG_MESSAGE, macos_message['type'], macos_message['subsystem'],
