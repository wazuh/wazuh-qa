...
@pytest.mark.parametrize('name, filetype, content, checkers, tags_to_apply, encoding', [
    ('file', REGULAR, 'Sample content', {CHECK_ALL}, {'wazuh_conf'}, None),
    ('file2', REGULAR, b'Sample content', {CHECK_ALL}, {'wazuh_conf'}, None),
    ('socketfile', REGULAR if sys.platform == 'win32' else SOCKET, '', {CHECK_ALL}, {'wazuh_conf'}, None),
    ('file3', REGULAR, 'Sample content', {CHECK_ALL}, {'wazuh_conf'}, None),
    ('fifofile', REGULAR if sys.platform == 'win32' else FIFO, '', {CHECK_ALL}, {'wazuh_conf'}, None),
    ('file4', REGULAR, '', {CHECK_ALL}, {'wazuh_conf'}, None),
    ('file-ñ', REGULAR, b'', {CHECK_ALL}, {'wazuh_conf'}, None),
    pytest.param('檔案', REGULAR, b'', {CHECK_ALL}, {'wazuh_conf'}, 'cp950', marks=(pytest.mark.linux,
                                                                                  pytest.mark.darwin,
                                                                                  pytest.mark.sunos5)),
    pytest.param('Образецтекста', REGULAR, '', {CHECK_ALL}, {'wazuh_conf'}, 'koi8-r', marks=(pytest.mark.linux,
                                                                                             pytest.mark.darwin,
                                                                                             pytest.mark.sunos5)),
    pytest.param('Δείγμακειμένου', REGULAR, '', {CHECK_ALL}, {'wazuh_conf'}, 'cp737', marks=(pytest.mark.linux,
                                                                                             pytest.mark.darwin,
                                                                                             pytest.mark.sunos5)),
    pytest.param('نصبسيط', REGULAR, '', {CHECK_ALL}, {'wazuh_conf'}, 'cp720', marks=(pytest.mark.linux,
                                                                                     pytest.mark.darwin,
                                                                                     pytest.mark.sunos5)),
    pytest.param('Ξ³ΞµΞΉΞ±', REGULAR, '', {CHECK_ALL}, {'wazuh_conf'}, None,
