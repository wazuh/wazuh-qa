...
def check_event(previous_mode: str, previous_event: dict, file: str):
    ...
    current_event = None
    try:
        TestLogger.VV('Checking for a second event...')
...
def test_duplicate_entries(get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    ...
    TestLogger.VV(f'Adding file {os.path.join(testdir1, file)}, content: " "')
...
def test_duplicate_entries_sregex(get_configuration, configure_environment,
    ...
    # Check for a second event
    TestLogger.VV(f'Modifying {os.path.join(testdir1, file)} content')
...
def test_duplicate_entries_complex(get_configuration, configure_environment, restart_syscheckd,
    ...
    TestLogger.VV(f'Adding file {os.path.join(testdir1, file)}, content: "testing"')
    ...
    TestLogger.VV(f'Replacing a character of {os.path.join(testdir1, file)} content')
    replace_character('i', '1', file_path)
    TestLogger.VV(f'Modifying {os.path.join(testdir1, file)}\'s group')
    modify_file_group(testdir1, file)
    TestLogger.VV(f'Modifying {os.path.join(testdir1, file)}\'s owner')
    modify_file_owner(testdir1, file)
    TestLogger.VV(f'Adding new file {file_path}')
    ...
    TestLogger.VV(f'Modifying {os.path.join(testdir1, file)}\'s permissions')
