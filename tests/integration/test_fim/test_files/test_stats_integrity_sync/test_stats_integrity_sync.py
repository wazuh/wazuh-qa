...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       these files are modified. Specifically, these tests will check the overall performance of
       the statistics recollection by the Wazuh daemons related to the FIM module.
...
def test_initialize_stats_collector(fim_eps, sync_eps, files, directory, buffer, case, modify_file, modify_all,
    ...
    '''
    description: Check the overall performance in terms of statistics recollection by the Wazuh daemons related
                 to the FIM module. For this purpose, the test will initialize several agents that send to
                 the manager a variable load of events (sync and FIM events). Then, it will collect the
                 metrics generated by the 'wazuh-db', 'wazuh-analysisd', 'wazuh-remoted' daemons, and the
                 agents. Finally, the test will store those metrics in several CSV files.
    ...
    parameters:
        - fim_eps:
            type: str
            brief: Number of FIM events per second to generate.
        - sync_eps:
            type: str
            brief: Number of synchronization events per second to generate.
        - files:
            type: str
            brief: Number of testing files to be created.
        - directory:
            type: str
            brief: Path to the directoy to be monitored.
        - buffer:
            type: str
            brief: Enable the agent buffer to avoid events flooding.
        - case:
            type: int
            brief: ID of the test case to be tested.
        - modify_file:
            type: bool
            brief: True for modify the checksum of a testing file. False otherwise.
        - modify_all:
            type: bool
            brief: True for modify all checksums in the database. False otherwise.
        - restore_all:
            type: bool
            brief: True if all entries in the 'fim_entry' table should be deleted. False otherwise.
        - initial_clean:
            type: fixture
    ...
    assertions:
        - Benchmark the statistics recollection by the Wazuh daemons related to the FIM module
          and collect the generated metrics to store them in CSV files.

    input_description: A template is used for the main configuration of the agent. It is
                       included in an external '.conf' file (template_agent.conf).
                       The test cases are defined in the module.

    expected_output:
        - A CSV file with the metrics collected from the 'wazuh-db' daemon.
        - A CSV file with the metrics collected from the 'wazuh-analysisd' daemon.
        - A CSV file with the metrics collected from the 'wazuh-remoted' daemon.
        - A CSV file with the metrics collected from the agents.
        - A CSV file with the metrics collected from the '.state' files.
