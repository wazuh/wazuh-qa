...
from wazuh_testing.tools.file import create_file_with_user, modify_file_content
from wazuh_testing.tools.monitoring import FileMonitor
from wazuh_testing.modules.fim import FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS as local_internal_options
from wazuh_testing.modules.fim import TEST_DIR_1
from wazuh_testing.modules.fim.event_monitor import callback_detect_file_added_event, callback_detect_file_modified_event
...
def display_file_owner(file_path):
    try:
        ...
        print(f"The owner of {file_path} is {domain}\\{name}")
    except Exception as e:
        print(f"Error displaying owner of {file_path}: {e}")
...
def test_reports_file_and_nodiff(configuration, metadata, set_wazuh_configuration,
    ...
    create_file_with_user(os.path.join(test_directories[0], file_name), metadata['username'])

    # Get log
    result = wazuh_log_monitor.start(timeout=T_20, callback=callback_detect_file_added_event, error_message='Error').result()
    display_file_owner(os.path.join(test_directories[0], file_name))
    modify_file_content(test_directories[0], file_name)
    display_file_owner(os.path.join(test_directories[0], file_name))
    result = wazuh_log_monitor.start(timeout=T_20, callback=callback_detect_file_modified_event, error_message='Error').result()
