...
from wazuh_testing.fim import LOG_FILE_PATH, callback_ignore, create_registry, delete_registry, registry_parser, \
        KEY_WOW64_32KEY, KEY_WOW64_64KEY, generate_params, callback_detect_event, check_time_travel, modify_registry_value
...
def test_ignore_registry_key(root_key, registry, subkey, arch, triggers_event, tags_to_apply,
    ...
    if triggers_event:
        ...
        assert event[0]['data']['type'] == 'added', f'Wrong event type.'
        assert event[0]['data']['path'] == os.path.join(root_key, registry, subkey), f'Wrong key path.'
        assert event[0]['data']['arch'] == '[x32]' if arch == KEY_WOW64_32KEY else '[x64]', f'wrong key arch.'

        assert event[1]['data']['type'] == 'modified', f'Parent key event type not equal'
        assert event[1]['data']['path'] == os.path.join(root_key, registry), f'Wrong parent key path.'
        assert event[1]['data']['arch'] == '[x32]' if arch == KEY_WOW64_32KEY else '[x64]', f'Parent key arch not equal.'
    else:
        ...
        assert event['data']['type'] == 'modified', f'Parent key event type not equal'
        assert event['data']['path'] == os.path.join(root_key, registry), f'Wrong parent key path.'
        assert event['data']['arch'] == '[x32]' if arch == KEY_WOW64_32KEY else '[x64]', f'Parent key arch not equal.'
...
def test_ignore_registry_value(root_key, registry, arch, value, triggers_event, tags_to_apply,
    ...
    if triggers_event:
        ...
        assert event[0]['data']['type'] == 'modified', f'Parent key event type not equal'
        assert event[0]['data']['path'] == os.path.join(root_key, registry), f'Wrong parent key path.'
        assert event[0]['data']['arch'] == '[x32]' if arch == KEY_WOW64_32KEY else '[x64]', f'Parent key arch not equal.'

        assert event[1]['data']['type'] == 'added', f'Wrong event type.'
        assert event[1]['data']['path'] == os.path.join(root_key, registry), f'Wrong value path.'
        assert event[1]['data']['arch'] == '[x32]' if arch == KEY_WOW64_32KEY else '[x64]', f'wrong key arch.'
        assert event[1]['data']['value_name'] == value, f'Wrong value name'
