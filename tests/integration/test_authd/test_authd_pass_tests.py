...
conf_params = {'USE_PASSWORD': []}

for case in pass_tests:
    conf_params['USE_PASSWORD'].append(case.get('USE_PASSWORD', DEFAULT_USE_PASSWORD))

p, m = generate_params(extra_params=conf_params, modes=['scheduled'] * len(pass_tests))
...
def test_authd_pass(get_configuration, configure_environment, configure_sockets_environment):
    ...
    set_password = None
    test_case = pass_tests[current_test]['test_case']
    try:
        if pass_tests[current_test]['USE_PASSWORD'] == 'yes':
            set_password = 'defined'
            if pass_tests[current_test]['random_pass'] == 'yes':
                set_password = 'random'
    except KeyError:
        pass
            ...
            if config['insert_prev_agent'] == "yes":
                ...
                assert response, \
                    'Failed connection previous insert for {}: {}'.format \
                        (pass_tests[current_test]['name'], config['input'])
