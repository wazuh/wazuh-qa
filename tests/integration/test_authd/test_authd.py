...
from wazuh_testing.tools.logging import FunctionLogger, TestLogger
...
def test_ossec_auth_messages(get_configuration, set_up_groups, configure_environment, configure_sockets_environment,
    ...
    for stage in test_case:
        TestLogger.V(f"Stage {stage}")
        # Reopen socket (socket is closed by manager after sending message with client key)
        receiver_sockets[0].open()
        expected = stage['output']
        TestLogger.V(f"Expected {expected}")
        message = stage['input']
        TestLogger.V(f"Send message {expected}")
        receiver_sockets[0].send(message, size=False)
        timeout = time.time() + 10
        response = ''
        while response == '':
            response = receiver_sockets[0].receive().decode()
            TestLogger.VV(f"Receive {response}")
