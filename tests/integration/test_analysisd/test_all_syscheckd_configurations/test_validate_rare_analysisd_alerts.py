...
brief: These tests will verify if the `wazuh-analysisd` daemon generates valid alerts from
       Linux `syscheck` events considered rare. The `wazuh-analysisd` daemon receives
       the log messages and compares them to the rules. It then creates an alert
       when a log message matches an applicable rule.
...
def test_validate_all_linux_alerts(configure_sockets_environment, connect_to_sockets_module, wait_for_analysisd_startup,
    ...
    inputs: 12298 test cases distributed among `syscheck` events of type `Added`, `Modified`, and `Deleted`.
