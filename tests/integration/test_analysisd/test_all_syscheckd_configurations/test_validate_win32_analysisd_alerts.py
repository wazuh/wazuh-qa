...
brief: These tests will verify if the `wazuh-analysisd` daemon generates valid alerts from
       Windows `syscheck` events. The `wazuh-analysisd` daemon receives the log messages
...
def test_validate_all_win32_alerts(configure_sockets_environment, connect_to_sockets_module, wait_for_analysisd_startup,
    ...
    inputs: 12272 test cases distributed among `syscheck` events of type `Added`, `Modified`, and `Deleted`.
