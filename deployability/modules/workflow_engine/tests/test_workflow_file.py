...
def test_workflow_file_constructor():
    ...
    with patch("workflow_engine.workflow_processor.WorkflowFile._WorkflowFile__validate_schema") as validate_mock, \
        ...
        wf = WorkflowFile(workflow_file_path='my_file.yml', schema_path='my_schema.yml')
    assert wf.schema_path == 'my_schema.yml'
    validate_mock.assert_called_once_with('my_file.yml')
    load_mock.assert_called_once_with('my_file.yml')
...
def test_workflow_file_validate_schema(logger_mock: MagicMock):
    ...
    wf = MagicMock()
    wf.schema_path = 'my_schema_path.yml'
    workflow_file = 'my_file_path.yml'
...
def test_workflow_file_expand_task(task: dict, return_value: dict, variables):
...
def test_workflow_file_static_workflow_validation_ko(task_collection: dict, error_msg: str):
    """Test WorkflowFile.__static_workflow_validation."""
    wf = MagicMock()
