...
from modules.provision.provision import Provision, PATH_BASE_DIR
...
def test_provision_run(logger_mock: MagicMock, provision_mock: Provision, stats: List[dict]):
    ...
    with patch.object(provision_mock, 'install_host_dependencies') as install_deps_mock, \
         patch.object(provision_mock, '_Provision__provision',
                      side_effect=lambda c: provision_mock.summary.update(stats[c.component])) as provision_method_mock:
        provision_mock.run()

    install_deps_mock.assert_called_once()
...
def test_provision_run_fail(logger_mock: MagicMock, provision_mock: Provision):
    ...
    with patch.object(provision_mock, 'install_host_dependencies'), \
         patch.object(provision_mock, '_Provision__provision',
...
@pytest.mark.parametrize('logger_mock, provision_mock',
                         [({'logger_to_patch': 'modules.provision.provision.logger'}, {})],
                         indirect=['logger_mock', 'provision_mock'])
def test_provision_install_host_dependencies(logger_mock: MagicMock, provision_mock: Provision):
    """Test Provision.install_host_dependencies method."""
    ...
    with patch('modules.provision.provision.Action', return_value=action) as action_mock, \
         patch('modules.provision.provision.ComponentInfo', return_value=component_info) as component_info_mock:
        provision_mock.install_host_dependencies()
    component_info_mock.assert_called_once_with(component=str(deps_path), type="dependencies")
    action_mock.assert_called_once_with("install", component_info, provision_mock.ansible_data)
    logger_mock.debug.assert_called_once_with(f"Installing dependencies on guests: {component_info}")
