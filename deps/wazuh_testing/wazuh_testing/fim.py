...
def create_registry(key, subkey, arch):
    ...
    if sys.platform == 'win32':
        try:
            ...
            FunctionLogger.V(f"Registry could not be created: {e}")
...
def delete_file(path, name):
    ...
    """
    FunctionLogger.VV(f"Removing file {str(os.path.join(path, name))}")
    regular_path = os.path.join(path, name)
...
def delete_registry(key, subkey, arch):
    ...
    if sys.platform == 'win32':
        ...
        FunctionLogger.VV("Removing registry key" +
        ...
        try:
            ...
            FunctionLogger.V("Couldn't remove registry key " +
...
def delete_registry_value(key_h, value_name):
    ...
    """
    if sys.platform == 'win32':
        FunctionLogger.VV(f"Removing registry value {value_name}.")

        try:
            win32api.RegDeleteValue(key_h, value_name)
        except (OSError, pywintypes.error) as e:
            FunctionLogger.V(f"Couldn't remove registry value {value_name}: {e}")
...
def modify_registry_value(key_h, value_name, type, value):
    ...
    if sys.platform == 'win32':
        try:
            FunctionLogger.VV(f"Modifying value '{value_name}' of type" + 
                            f"{registry_value_type[type]} and value '{value}'")

            win32api.RegSetValueEx(key_h, value_name, 0, type, value)
        except (OSError, pywintypes.error) as e:
            FunctionLogger.V(f"Could not modify registry value content: {e}")
...
def modify_key_perms(key, subkey, arch, user):
    ...
    if sys.platform == 'win32':
        print_arch = '[x64]' if arch == KEY_WOW64_64KEY else '[x32]'
        FunctionLogger.VV(f"- Changing permissions of" +
...
def modify_registry_key_mtime(key, subkey, arch):
    ...
    if sys.platform == 'win32':
        print_arch = '[x64]' if arch == KEY_WOW64_64KEY else '[x32]'
        FunctionLogger.VV('- Changing mtime of ' +
                        f"{print_arch}{os.path.join(registry_class_name[key], subkey)}")
        ...
        try:
            ...
            FunctionLogger.V(f"Registry mtime could not be modified: {e}")
...
def modify_registry_owner(key, subkey, arch, user):
    ...
    if sys.platform == 'win32':
        print_arch = '[x64]' if arch == KEY_WOW64_64KEY else '[x32]'
        FunctionLogger.VV(f'- Changing owner of' +
                        f"{print_arch}{os.path.join(registry_class_name[key], subkey)}")
        ...
        try:
            ...
            FunctionLogger.V(f"Registry owner could not be modified: {e}")
...
def modify_registry(key, subkey, arch):
    ...
    """
    print_arch = '[x64]' if arch == KEY_WOW64_64KEY else '[x32]'
    FunctionLogger.VV('Modifying registry key '
                    f"{print_arch}{os.path.join(registry_class_name[key], subkey)}")
...
def modify_file_content(path, name, new_content=None, is_binary=False):
    ...
    FunctionLogger.VV("- Changing content of " + str(path_to_file))
...
def modify_file_mtime(path, name):
    ...
    """
    path_to_file = os.path.join(path, name)
    FunctionLogger.VV("- Changing mtime of " + str(path_to_file))
...
def modify_file_owner(path, name):
    ...
    path_to_file = os.path.join(path, name)
    FunctionLogger.VV("- Changing owner of " + str(path_to_file))
...
def modify_file_group(path, name):
    ...
    path_to_file = os.path.join(path, name)
    FunctionLogger.VV("- Changing group of " + str(path_to_file))
    os.chown(path_to_file, -1, 1)
...
def modify_file_permission(path, name):
    ...
    FunctionLogger.VV("- Changing permission of " + str(path_to_file))
...
def modify_file_inode(path, name):
    ...
    FunctionLogger.VV("- Changing inode of " + str(os.path.join(path, name)))
    inode_file = 'inodetmp'
    path_to_file = os.path.join(path, name)
...
def modify_file_win_attributes(path, name):
    ...
    FunctionLogger.VV("- Changing win attributes of " + str(os.path.join(path, name)))
...
def modify_file(path, name, new_content=None, is_binary=False):
    ...
    """
    FunctionLogger.VV("Modifying file " + str(os.path.join(path, name)))
...
def callback_detect_end_scan(line):
    ...
    try:
        ...
        FunctionLogger.V(f"Couldn't load a log line into json object. Reason {e}")
...
def check_time_travel(time_travel: bool, interval: timedelta = timedelta(hours=13), monitor: FileMonitor = None,
    ...
    if time_travel:
        ...
        FunctionLogger.VV(f"Changing the system clock from {before} to {str(datetime.now())}")
...
class EventChecker:
    ...
    def fetch_events(self, min_timeout=1, triggers_event=True, extra_timeout=0, error_message=''):
        ...
        try:
            ...
            if triggers_event:
                raise
            FunctionLogger.VV('TimeoutError was expected and correctly caught.')
...
def wait_for_scheduled_scan(wait_for_scan=False, interval: timedelta = timedelta(seconds=20), monitor: FileMonitor = None,
    ...
    if wait_for_scan:
        FunctionLogger.VV(f"waiting for scheduled scan to start for {interval} seconds")
        time.sleep(interval)
        if monitor:
            monitor.start(timeout=timeout, callback=callback_detect_end_scan,
                        update_position=False,
...
if sys.platform == 'win32':
    ...
    def registry_value_cud(root_key, registry_sub_key, log_monitor, arch=KEY_WOW64_64KEY, value_list=['test_value'],
        ...
        if triggers_event_add:
            FunctionLogger.VV("'added' {} detected as expected.\n".format("events"
        ...
        if triggers_event_modified:
            FunctionLogger.VV("'modified' {} detected as expected.\n".format("events"
                            if len(value_list) > 1 else "event"))
        ...
        if triggers_event_delete:
            FunctionLogger.VV("'deleted' {} detected as expected.\n".format("events"
                            if len(value_list) > 1 else "event"))
        # Update the position of the log to the end of the scan
        if time_travel:
            log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_end_scan,
