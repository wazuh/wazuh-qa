...
from wazuh_testing.cmt import BINARY_PATH, CVE5_SCHEMA_PATH,DELTA_SCHEMA_PATH, REPORT_ERROR_MESSAGE, SNAPSHOTS_DIR
...
def run_content_migration_tool(args):
    """Run the CMT with specified parameters and get the output.
...
def query_publisher_db(query):
    """Function to query the DB created by CMT.
...
def clean_cmt_output_files():
    """Remove all files generated by CMT.
    """
    folders_list = glob.glob(os.path.join(SNAPSHOTS_DIR, '*'))
