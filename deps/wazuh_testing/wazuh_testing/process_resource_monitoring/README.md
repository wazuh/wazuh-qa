...
These Python modules comply with a series of standards, among which it is worth highlighting PEP 8 standard.   
You can check the compliance of Python modules using ruff with the provided configuration.
...
sh -c "mkdir -p .conf/ruff/ && cat <<EOL > .conf/ruff/pyproject.toml
        [tool.pyright]
        ...
        [tool.ruff.lint]
        select = ["F", "I", "W", "E", "C90", "N", "D", "UP", "CPY", "YTT", "B", "T20"]
...
# Check a python file overriding default configuration
ruff check file.py --config .conf/ruff/pyproject.toml --preview
```
