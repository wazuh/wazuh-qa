...
class Agent:
    ...
    def processCommand(self, sender, message_list):
        ...
        if command in ['lock_restart', 'open', 'write', 'close', 'clear_upgrade_result']:
            ...
            # !-req num ok {sha}
            if self.sha_key:
                sender.sendEvent(self.createEvent(f'#!-req {message_list[1]} ok {self.sha_key}'))
            else:
...
def create_agents(agents_number, manager_address, cypher, fim_eps=None, authd_password=None, os=None):
    ...
    for agent in range(agents_number):
        ...
        if authd_password != None:
            agents.append(Agent(manager_address, cypher, fim_eps=fim_eps, authd_password=authd_password, os=agent_os))
        else:
            agents.append(Agent(manager_address, cypher, fim_eps=fim_eps, os=agent_os))
