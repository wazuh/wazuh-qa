...
class StatisticMonitor:
    ...
    def _parse_logcollector_state_file(self, data):
        ...
        for file in state_info['global']['files']:
            ...
            for target in file['targets']:
                ...
                self._write_csv(file_data, 'logcollector_state' join(self.dst_dir, f'{csv_name}.csv'))


    def _parse_state_file(self, target):
        ...
        try:
            ...
            if not self.parse_json:
                ...
                self._parse_logcollector_state_file(data, target)
        except Exception as e:
            logger.error(f'Exception with {self.statistics_file} | {str(e)}')


    def _parse_api_data(self, target='analysisd_events'):
    ...
    if target == "analysisd_state":
        csv_header = analysisd_header
    elif target == "analysisd_events":
        csv_header = analysisd_events_header
    elif target == "logcollector_state":
