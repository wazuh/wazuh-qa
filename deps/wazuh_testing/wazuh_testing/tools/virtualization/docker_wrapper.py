...
class DockerWrapper():
    ...
    def restart(self):
        ...
        try:
            ...
            DockerWrapper.LOGGER.debug(
                f'The {self.name} cointainer has been restarted sucessfully')
    ...
    def halt(self):
        ...
        try:
            ...
            DockerWrapper.LOGGER.debug(
                f'The {self.name} cointainer has been stopped sucessfully')
        except docker.errors.NotFound:
            pass
    ...
    def destroy(self, remove_image=False):
        ...
        try:
            ...
            DockerWrapper.LOGGER.debug(
                f'The {self.name} cointainer has been removed sucessfully')
        except docker.errors.NotFound:
            pass

        if remove_image:
            DockerWrapper.LOGGER.debug(
                f'Removing {self.image.id} docker image')
            self.docker_client.images.remove(image=self.image.id, force=True)
            DockerWrapper.LOGGER.debug(
                f'The {self.image.id} image has been removed sucessfully')
    ...
    def execute(self, command: str) -> str:
        ...
        try:
            ...
            DockerWrapper.LOGGER.debug(
                f'Command {command} executed with exit_code {exit_code}')
            return output.decode('UTF-8')
        except docker.errors.APIError as e:
            DockerWrapper.LOGGER.error(
                f'An internal error in cointainer {self.name}.')
            return f'Error from the docker server: {e}'
        except docker.errors.NotFound as e:
            DockerWrapper.LOGGER.error(
                f'The container {self.name} not exists.')
