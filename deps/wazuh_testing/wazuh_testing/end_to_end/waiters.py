...
def wait_until_vd_is_updated(host_manager):
    ...
    for manager in host_manager.get_group_hosts('manager'):
        monitoring_data = generate_monitoring_logs_manager(host_manager, manager, 'Starting vulnerability scan', 800)
...
def wait_until_vuln_scan_agents_finished(host_manager):
    for agent in host_manager.get_group_hosts('agent'):
        ...
        monitoring_data = generate_monitoring_logs_manager(host_manager, manager_host, rf"Finished vulnerability assessment for agent '{agents_id[agent]}'", 700)
        monitoring_events_multihost(host_manager, monitoring_data)
