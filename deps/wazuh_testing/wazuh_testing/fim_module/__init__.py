...
CB_IGNORING_PATH_DUE_TO_SREGEX_OR_PATTERN = (
    r".*?Ignoring path '(.*)' due to (sregex|pattern)? '(.*)'.*"
)

# Error Messages
ERR_MSG_DATABASE_PERCENTAGE_FULL_ALERT = (
    'Did not receive expected "DEBUG: ...: Sending DB ...% full alert." event'
)
ERR_MSG_FIM_INODE_ENTRIES = (
    'Did not receive expected "Fim inode entries: ..., path count: ..." event'
)
ERR_MSG_DB_BACK_TO_NORMAL = (
    'Did not receive expected "DEBUG: ...: Sending DB back to normal alert." event'
)
