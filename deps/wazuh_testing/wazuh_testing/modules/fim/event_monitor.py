...
def callback_entries_path_count(line):
    if platform != 'win32':
        match = re.match(fim.CB_INODE_ENTRIES_PATH_COUNT, line)
    else:
        match = re.match(fim.CB_FIM_ENTRIES_COUNT, line)
...
def callback_num_inotify_watches(line):
    ...
    """
    match = re.match(fim.CB_REALTIME_MONITORED_FOLDERS, line)
...
def callback_real_time_whodata_started(line):
    ...
    Args:
        line (String): string line to be checked by callback in File_Monitor.
    """
    if fim.CB_REALTIME_WHODATA_ENGINE_STARTED in line:
        return True
