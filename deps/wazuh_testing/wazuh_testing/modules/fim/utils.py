...
from wazuh_testing import global_parameters, logger, REGULAR, LOG_FILE_PATH
...
def get_sync_msgs(timeout, new_data=True):
    ...
    if new_data:
        wazuh_log_monitor.start(timeout=timeout,
                                callback=generate_monitoring_callback(fim.CB_REGISTRY_DBSYNC_NO_DATA),
...
def regular_file_cud(folder, log_monitor, file_list=['testfile0'], time_travel=False, min_timeout=1, options=None,
                     triggers_event=True, encoding=None, callback=callback_detect_event,
