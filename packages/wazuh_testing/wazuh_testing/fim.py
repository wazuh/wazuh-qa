...
def validate_event(event, checks=None, trigger_expected=True):
    ...
    is_triggered = attributes ^ required_attributes == set()

    # Check audit
    if event['data']['mode'] == 'whodata':
        is_triggered &= 'audit' in event['data']
        is_triggered &= event['data']['audit'].keys(
        ) ^ _REQUIRED_AUDIT == set()

    assert(is_triggered == trigger_expected)
...
def create_file(type, name, path, content=''):
    """ Creates a file in a given path. The path will be create in case it does not exists.
...
def callback_audit_added_rule(line):
    match = re.match(
        r'.*Added audit rule for monitoring directory: \'(.+)\'', line)
    if match:
...
def callback_realtime_added_directory(line):
    match = re.match(
        r'.*Directory added for real time monitoring: \'(.+)\'', line)
    if match:
        return match.group(1)
    return None
...
def regular_file_cud(folder, time_travel, n_regular, min_timeout, log_monitor, options=None, should_be_triggered=True):
    ...
    def check_events_type(ev_type):
        if events:
            event_types = Counter(jq(".[].data.type").transform(
                events, multiple_output=True))
            was_triggered = (event_types[ev_type] == n_regular)
        else:
            was_triggered = False
        assert(was_triggered == should_be_triggered)

    def check_files_in_event():
        if events:
            # Are the n_regular events the files modified?
            file_paths = jq(".[].data.path").transform(
                events, multiple_output=True)
            for file_name in range(n_regular):
                was_triggered = os.path.join(
                    folder, f'regular_{file_name}') in file_paths
                assert (was_triggered == should_be_triggered)
        else:
            assert(False == should_be_triggered)
